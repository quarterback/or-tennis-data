...
<head>
    ...
    <style>
        ...
        .sd1-banner:hover, .sd1-banner:focus {{ filter: brightness(1.08); color: #fff; }}
        .sd1-banner-tag {{
        ...
        .sd1-banner-text {{ flex: 1; }}
        .sd1-banner-arrow {{ font-size: 16px; opacity: 0.85; }}
        @media (max-width: 600px) {{
            .sd1-banner {{ font-size: 13px; padding: 8px 12px; }}
            .sd1-banner-text {{ font-size: 12px; }}
        }}
        ...
        <div class="toolbar">
            ...
            <div class="filter-group">
                ...
                <div class="sort-toggle">
                    <button class="btn btn-outline-secondary btn-sm active" id="sortRank">Rank</button>
                    <button class="btn btn-outline-secondary btn-sm" id="sortPowerIndex">Power Index</button>
            ...
            <div class="playoff-toolbar" style="margin-top:10px; padding-top:10px; border-top:1px solid #dee2e6;">
                <div class="form-group">
                    ...
                    <select id="bracketMode" class="form-select form-select-sm">
                        ...
                        <option value="osaa">OSAA Bracketing (3-Move Same-League Avoidance)</option>
                    </select>
                </div>
                ...
                <div id="osaaInfo" class="form-group" style="display:none;">
                    <small class="text-muted">
                        <strong>OSAA Bracketing:</strong> Follows official OSAA rules. Attempts up to 3 visitor swaps (downward preferred) to avoid same-league first-round matchups. Never removes home games.
                    </small>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {{
            ...
            table = $('#rankingsTable').DataTable({{
                ...
                order: [[0, 'asc']],  // State Rank column (ascending, reflects H2H boosts)
                pageLength: 50,
            ...
            $('#sortRank').on('click', function() {{
                ...
                table.order([[0, 'asc']]).draw();  // State Rank is column 0
            }});

            $('#sortPowerIndex').on('click', function() {{
                $(this).addClass('active');
                $('#sortRank, #sortAPR').removeClass('active');
        ...
        // Generate OSAA bracketing matchups with 3-move same-league avoidance
        // Implements official OSAA rules for avoiding same-league first-round matchups
        function generateOSAAMatchups(field) {{
            const bracketSize = field.length;
            let matchups = [];

            // Start with pure seeding matchups
            if (bracketSize === 16) {{
                // Initial matchups: 1v16, 2v15, 3v14, ..., 8v9
                const hosts = field.slice(0, 8);
                const visitors = field.slice(8, 16);
                ...
                for (let i = 0; i < hosts.length; i++) {{
                    matchups.push({{
                        ...
                        seed2: 16 - i,
                        team2: visitors[7 - i],
                        distance: calcDistance(hosts[i].coords, visitors[7 - i]?.coords)
                    }});
                }}
            }} else if (bracketSize === 12) {{
                // 12-team: seeds 1-4 have byes, 5v12, 6v11, 7v10, 8v9
                const hosts = field.slice(4, 8);  // Seeds 5-8
                const visitors = field.slice(8, 12);  // Seeds 9-12
                ...
                for (let i = 0; i < hosts.length; i++) {{
                    matchups.push({{
                        ...
                        seed2: 12 - i,
                        team2: visitors[3 - i],
                        distance: calcDistance(hosts[i].coords, visitors[3 - i]?.coords)
                    }});
                }}
            }} else if (bracketSize === 8) {{
                // 8-team: 1v8, 2v7, 3v6, 4v5
                const hosts = field.slice(0, 4);  // Seeds 1-4
                const visitors = field.slice(4, 8);  // Seeds 5-8
            ...
            // Apply OSAA 3-move rule to resolve same-league conflicts
            // Rule: Always move the lower ranked team (visitor), never take away home games
            const visitorStartSeed = bracketSize === 12 ? 9 : (bracketSize === 16 ? 9 : 5);
            const visitorEndSeed = bracketSize;

            for (let i = 0; i < matchups.length; i++) {{
                const matchup = matchups[i];

                // Check if there's a same-league conflict
                if (!sameLeague(matchup.team1, matchup.team2)) {{
                    matchup.conflict = false;
                    continue;
                }}

                // Try to resolve conflict with up to 3 moves
                let resolved = false;
                const originalVisitorSeed = matchup.seed2;

                // Try moving downward first (to lower seeds)
                for (let move = 1; move <= 3 && !resolved; move++) {{
                    const targetSeed = originalVisitorSeed + move;

                    // Check if target seed is within visitor range
                    if (targetSeed > visitorEndSeed) break;

                    // Find the matchup that has this target seed as visitor
                    const targetMatchupIdx = matchups.findIndex(m => m.seed2 === targetSeed);
                    if (targetMatchupIdx === -1) continue;

                    const targetMatchup = matchups[targetMatchupIdx];

                    // Check if swapping would resolve current conflict without creating new one
                    if (!sameLeague(matchup.team1, targetMatchup.team2) &&
                        !sameLeague(targetMatchup.team1, matchup.team2)) {{
                        // Perform the swap
                        const temp = matchup.team2;
                        matchup.team2 = targetMatchup.team2;
                        matchup.distance = calcDistance(matchup.team1.coords, matchup.team2?.coords);
                        matchup.swapped = true;
                        matchup.swapMove = move;
                        matchup.swapDirection = 'down';
                ...
                // If not resolved by moving down, try moving up
                if (!resolved) {{
                    for (let move = 1; move <= 3 && !resolved; move++) {{
                        const targetSeed = originalVisitorSeed - move;

                        // Check if target seed is within visitor range
                        if (targetSeed < visitorStartSeed) break;
                ...
                // If still not resolved after 3 moves, mark as conflict but leave as is
                if (!resolved) {{
                    matchup.conflict = true;
                    matchup.unresolvable = true;
                }}
            }}
        ...
        function generatePlayoffFieldFromSelection() {{
            ...
            // Find all leagues with champions
            const leagueChampionIndices = new Map(); // league -> highest-ranked champion index
            field.forEach((team, idx) => {{
                if (team.qualifyType === 'auto' && team.league) {{
                    const currentChampIdx = leagueChampionIndices.get(team.league);
                    if (currentChampIdx === undefined || idx < currentChampIdx) {{
                        leagueChampionIndices.set(team.league, idx);
                    }}
                }}
            }});

            // Single-pass: Move at-large teams below their league champion if needed
            // Build corrected field without infinite loops
            const correctedField = [];
            const pendingAtLarge = []; // At-large teams that need to be moved down

            for (let i = 0; i < field.length; i++) {{
                const team = field[i];

                if (team.qualifyType === 'atlarge' && team.league && leagueChampionIndices.has(team.league)) {{
                    const champIdx = leagueChampionIndices.get(team.league);
                    if (i < champIdx) {{
                        // This at-large is ranked higher than their champion - hold it back
                        pendingAtLarge.push(team);
                        continue;
                    }}
                }}

                // Add team to corrected field
                correctedField.push(team);

                // If this is a league champion, insert any pending at-large teams from this league after them
                if (team.qualifyType === 'auto' && team.league) {{
                    const fromThisLeague = pendingAtLarge.filter(t => t.league === team.league);
                    fromThisLeague.forEach(t => {{
                        correctedField.push(t);
                        const idx = pendingAtLarge.indexOf(t);
                        pendingAtLarge.splice(idx, 1);
                    }});
                }}
            }}

            // Add any remaining pending at-large teams at the end
            correctedField.push(...pendingAtLarge);
            ...
            const osaaMatchups = bracketMode === 'osaa' ? generateOSAAMatchups(field) : pureMatchups;
            ...
            const osaaMileage = osaaMatchups.reduce((sum, m) => sum + (m.distance || 0), 0);
            ...
            if (bracketMode === 'osaa') {{
                // Show OSAA bracketing statistics
                const swappedCount = osaaMatchups.filter(m => m.swapped).length;
                const unresolvedCount = osaaMatchups.filter(m => m.unresolvable).length;
                const downMoves = osaaMatchups.filter(m => m.swapDirection === 'down').length;
                const upMoves = osaaMatchups.filter(m => m.swapDirection === 'up').length;

                if (unresolvedCount > 0) {{
                    html += `
                        <div class="alert alert-warning mb-3">
                            <strong>⚠️ OSAA Bracketing:</strong> ${{unresolvedCount}} same-league matchup(s) could not be resolved after 3 swap attempts
                            <br><small>Resolved: ${{swappedCount}} conflicts | Down moves: ${{downMoves}} | Up moves: ${{upMoves}}</small>
                        </div>
                    `;
                }} else if (swappedCount > 0) {{
                    html += `
                        <div class="alert alert-success mb-3">
                            <strong>✓ OSAA Bracketing:</strong> All same-league conflicts resolved
                            <br><small>Swaps made: ${{swappedCount}} | Down moves: ${{downMoves}} | Up moves: ${{upMoves}}</small>
                        </div>
                    `;
                }} else {{
                    html += `
                        <div class="alert alert-info mb-3">
                            <strong>✓ OSAA Bracketing:</strong> No same-league conflicts detected
                            <br><small>Pure seeding maintained</small>
                        </div>
                    `;
                }}
            }}
            ...
            const displayMatchups = bracketMode === 'regional' ? regionalMatchups :
                                   (bracketMode === 'osaa' ? osaaMatchups : pureMatchups);
            ...
            displayMatchups.forEach(m => {{
                const conflictBadge = m.conflict ? '<span class="badge bg-warning text-dark ms-1">⚠️ Same League</span>' : '';
                const optimizedBadge = m.optimized ? '<span class="badge bg-info text-white ms-1">📍 Optimized</span>' : '';
                const swappedBadge = m.swapped && !m.unresolvable ? `<span class="badge bg-success text-white ms-1">🔄 Swapped (${{m.swapDirection}})</span>` : '';
                const unresolvedBadge = m.unresolvable ? '<span class="badge bg-danger text-white ms-1">⚠️ Unresolved</span>' : '';
                ...
                html += `
                    <div class="matchup-row ${{tierClass}}" style="padding:8px 0; border-bottom:1px solid #eee;">
                        ...
                        ${{conflictBadge}}${{optimizedBadge}}${{swappedBadge}}${{unresolvedBadge}}
                    </div>
                `;
            }});
