    ...
    <script>
        ...
        function generateRegionalMatchups(field) {{
            ...
            if (bracketSize === 16) {{
                ...
                const topHalf = field.slice(0, 4); // Seeds 1-4
                const botHalf = field.slice(4, 8); // Seeds 5-8

                const usedBot = new Set();
                topHalf.forEach((topTeam, i) => {{
                    let bestMatch = null;
                    let bestDist = Infinity;
                    botHalf.forEach((botTeam, j) => {{
                        if (usedBot.has(j)) return;
                        if (sameDistrict(topTeam, botTeam)) return;
                        const dist = calcDistance(topTeam.coords, botTeam.coords);
                        if (dist < bestDist) {{
                            bestDist = dist;
                    ...
                    if (bestMatch) {{
                        usedBot.add(bestMatch.index);
                        matchups.push({{
                            seed1: i + 1, team1: topTeam,
                            seed2: 4 + bestMatch.index + 1, team2: bestMatch.team,
                            distance: bestMatch.dist,
                            tier: 'flex',
                            optimized: true
                        }});
        ...
        function generatePlayoffFieldFromSelection() {{
            ...
            if (bracketSize === 12) {{
                // Show bye teams and their QF opponents
                html += `<div class="section-title mt-3">Quarterfinal Matchups (Seeds 1-4 have byes)</div>`;
                html += `<div class="bg-white p-3 rounded shadow-sm mb-3">`;

                // Map first round winners to bye teams
                // Standard bracket: #1 vs winner(8v9), #2 vs winner(7v10), #3 vs winner(6v11), #4 vs winner(5v12)
                const byeTeams = field.slice(0, 4);
                const firstRoundMatchups = displayMatchups;

                // For each bye team, find the corresponding first round matchup
                // In standard: bye team i plays the winner of seeds (8-i) vs (9+i)
                // But in regional mode, matchups may differ, so we match by the lower seed
                byeTeams.forEach((byeTeam, i) => {{
                    // Standard opponent seeds for this bye team
                    const standardLowSeed = 5 + i;  // 5,6,7,8
                    const standardHighSeed = 12 - i; // 12,11,10,9

                    // Find the actual matchup that contains the expected seeds
                    const matchup = firstRoundMatchups.find(m =>
                        m.seed1 === standardLowSeed || m.seed2 === standardLowSeed
                    );
                    ...
                    if (matchup) {{
                        html += `
                            <div style="padding:8px 0; border-bottom:1px solid #eee;">
                                ...
                                <span class="text-muted mx-2">→ QF vs winner of</span>
                                <span class="matchup-seed">#${{matchup.seed1}}</span>
                                <strong>${{matchup.team1?.school_name}}</strong>
                                <span class="text-muted mx-1">vs</span>
                                <span class="matchup-seed">#${{matchup.seed2}}</span>
                                <strong>${{matchup.team2?.school_name}}</strong>
                            </div>
                        `;
                    }} else {{
                        html += `
